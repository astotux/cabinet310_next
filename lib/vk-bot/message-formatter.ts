...
export class MessageFormatter {
  ...
  formatCreatedBookingConfirmation(booking: VKBookingData): FormattedMessage {
      const phone = booking.master === "Лиза" ? "+7 (912) 509-59-09" : "+7 (908) 695-49-04";
...
Если у вас есть вопросы, обращайтесь к администратору! 😊`;
   ...
   */
  formatWelcome(activePromo?: { title: string; text: string } | null): FormattedMessage {
    let text = `👋 Добро пожаловать в студию красоты!\n\nЯ ваш помощник для записи на услуги. Здесь вы можете:\n\n🔸 Посмотреть прайс-лист\n🔸 Записаться на любую услугу\n🔸 Получить информацию о студии`;

    if (activePromo) {
      text += `\n\n🎉 АКЦИЯ: ${activePromo.title}\n${activePromo.text}`;
    }
